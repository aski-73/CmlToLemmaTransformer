...
public class DataDslExtractor {
  ...
  public String extractToString(final DataStructure dataStructure) {
    String _xblockexpression = null;
    {
      ...
      boolean setFieldOperationSeparator = ((dataStructure.getOperations().size() > 0) && (dataStructure.getDataFields().size() > 0));
      ...
      _builder_2.newLineIfNotEmpty();
      {
        EList<DataOperation> _operations = dataStructure.getOperations();
        ...
        for(final DataOperation op : _operations) {
          ...
          _builder_2.append("\t");
          String _extractToString_1 = this.extractToString(op);
          _builder_2.append(_extractToString_1, "\t");
          _builder_2.newLineIfNotEmpty();
        }
      }
      _builder_2.append("}");
  ...
  public String extractToString(final DataField dataField) {
    String _xblockexpression = null;
    {
      ...
      _builder.append(_lemmaName);
      String _extractFeatures = DataDslExtractor.extractFeatures(dataField.getFeatures());
   ...
   */
  public String extractToString(final DataOperation dataOperation) {
    final Object operationReturnType = this.operationReturnType(dataOperation);
    String _switchResult = null;
    boolean _matched = false;
    if (operationReturnType instanceof Type) {
      _matched=true;
      _switchResult = this.extractTypeReferenceToString(((Type)operationReturnType), Util.qualifiedName(dataOperation));
    }
    if (!_matched) {
      if (operationReturnType instanceof ImportedComplexType) {
        _matched=true;
        _switchResult = this.extractTypeReferenceToString(((ImportedComplexType)operationReturnType));
      }
    }
    if (!_matched) {
      _switchResult = "";
    ...
    final String operationKind = _xifexpression;
    boolean _isEmpty = dataOperation.getParameters().isEmpty();
    if (_isEmpty) {
      StringConcatenation _builder_1 = new StringConcatenation();
      _builder_1.append(operationKind);
      _builder_1.append(" ");
      String _lemmaName = Util.lemmaName(dataOperation.getName());
      _builder_1.append(_lemmaName);
      String _extractFeatures = DataDslExtractor.extractFeatures(dataOperation.getFeatures());
      _builder_1.append(_extractFeatures);
      _builder_1.append("()");
      return _builder_1.toString();
    } else {
      StringConcatenation _builder_2 = new StringConcatenation();
      _builder_2.append(operationKind);
      _builder_2.append(" ");
      String _lemmaName_1 = Util.lemmaName(dataOperation.getName());
      _builder_2.append(_lemmaName_1);
      String _extractFeatures_1 = DataDslExtractor.extractFeatures(dataOperation.getFeatures());
      _builder_2.append(_extractFeatures_1);
      _builder_2.append("(");
      {
        EList<DataOperationParameter> _parameters = dataOperation.getParameters();
        boolean _hasElements = false;
        for(final DataOperationParameter param : _parameters) {
          ...
          String _extractToString = this.extractToString(param);
          _builder_2.append(_extractToString);
        }
      }
      _builder_2.append(")");
   ...
   */
  private Object operationReturnType(final DataOperation dataOperation) {
    EObject _xifexpression = null;
    ComplexType _complexReturnType = dataOperation.getComplexReturnType();
    boolean _tripleNotEquals = (_complexReturnType != null);
    if (_tripleNotEquals) {
      _xifexpression = dataOperation.getComplexReturnType();
    } else {
      EObject _xifexpression_1 = null;
      PrimitiveType _primitiveReturnType = dataOperation.getPrimitiveReturnType();
      boolean _tripleNotEquals_1 = (_primitiveReturnType != null);
      if (_tripleNotEquals_1) {
        _xifexpression_1 = dataOperation.getPrimitiveReturnType();
      } else {
        ImportedComplexType _xifexpression_2 = null;
        ImportedComplexType _importedComplexReturnType = dataOperation.getImportedComplexReturnType();
        boolean _tripleNotEquals_2 = (_importedComplexReturnType != null);
        if (_tripleNotEquals_2) {
          _xifexpression_2 = dataOperation.getImportedComplexReturnType();
        } else {
   ...
   */
  public String extractToString(final DataOperationParameter param) {
    String _xblockexpression = null;
    {
      final Object directFieldType = this.paramType(param);
      String _switchResult = null;
      boolean _matched = false;
      if (directFieldType instanceof Type) {
        _matched=true;
        _switchResult = this.extractTypeReferenceToString(((Type)directFieldType), Util.qualifiedName(param));
      }
      if (!_matched) {
        if (directFieldType instanceof ImportedComplexType) {
          _matched=true;
      ...
      _builder.append(type);
      _builder.append(" ");
      String _lemmaName = Util.lemmaName(param.getName());
      _builder.append(_lemmaName);
      _xblockexpression = _builder.toString();
    }
    return _xblockexpression;
   ...
   */
  private static String extractFeatures(final EList<? extends Enumerator> features) {
    String _xifexpression = null;
    ...
    if (_greaterThan) {
      ...
      _builder.append("<");
      {
        ...
        for(final Enumerator feat : features) {
          ...
          String _transformFeatureToCamelCase = Util.transformFeatureToCamelCase(feat);
